...
export function UiModal({
  ...
  isOpen,
  onClose,
}:{width:string, className:string,isOpen:boolean,onClose:()=>void,children:React.ReactNode}) {
  const handlesCliek = (e:DOMEvent<HTMLInputElement>) => {
...
UiModal.Header = function UiModalHeader({  children, className  }:{className:string,children:React.ReactNode}) {
  return (
    <div className={clsx(className, "px-6 pt-6 pb-4 text-2xl")}>{children}</div>
  );
};

UiModal.Body = function UiModalBody({ className, children }:{className:string,children:React.ReactNode}) {
  return <div className={clsx(className, "px-6")}>{children}</div>;
};

UiModal.Footer = function UiModalFooter({  className, children }:{className:string,children:React.ReactNode}) {
  return (
